...
/*   Updated: 2022/12/10 22:31:44 by zkarapet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	func_for_reds(char *s, t_cmd *cmd_node)
{
	...
	while (s[++i])
	{
		...
		else if (s[i] == '>')
		{
			...
			if (s[i + 1] == '>')
				cmd_node->fd_in = open(less_red(&s[i + 2], O_WRONLY | O_APPEND | O_CREATE);
			else
			//dup2()
				cmd_node->fd_in = open(less_red(&s[i + 1], O_WRONLY | O_TRUNC | O_CREATE);
		}
	}
}
